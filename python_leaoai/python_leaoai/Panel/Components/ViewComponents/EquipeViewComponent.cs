...
{
    public class EquipeViewComponent : ViewComponent
    {
        private static ApplicationDbContext _context;
        public EquipeViewComponent(ApplicationDbContext context)
        {
            _context = context;
        }
        public async Task<IViewComponentResult> InvokeAsync(string id)
        {
            ...
            if (ID > 0)
                _context.Equipe.Where(b => b.Ativo == true && b.Id == ID).ToList().ForEach(obj =>
                {
                    dropItens.Add(new DropItem(obj.Titulo, obj.Id.ToString()));
                });
            else
                _context.Equipe.Where(b => b.Ativo == true).ToList().ForEach(obj =>
                {
                    dropItens.Add(new DropItem(obj.Titulo, obj.Id.ToString()));
                });
