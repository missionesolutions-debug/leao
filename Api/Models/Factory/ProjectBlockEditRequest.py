...
class ProjectBlockEditRequest(BaseModel):
    ...
    measure_items: List[ProjectMeasureItemRequest] = Field(alias='MeasureItems', Field(default_factory=list)) # Corresponds to public List<ProjectMeasureItemRequest> MeasureItems
    files: List[IFormFile] = Field(alias='Files', Field(default_factory=list)) # Corresponds to public List<IFormFile> Files
