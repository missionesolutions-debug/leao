...
from app.schemas.caducidade_schema import CaducidadeCreate, CaducidadeResponse
...
def atualizar_caducidade(caducidade_id: int, caducidade: CaducidadeCreate, db: Session = Depends(get_db)):
    db_obj = db.query(Caducidade).filter(Caducidade.id == caducidade_id).first()
    if not db_obj:
        raise HTTPException(status_code=404, detail="Caducidade não encontrada")
    for key, value in caducidade.dict().items():
